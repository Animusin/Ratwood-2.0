...
/mob/living/proc/bleed(amt)
	...
	if(mind)
		...
		if(HAS_TRAIT(src, TRAIT_BLOOD_RESISTANCE))//this comes first so it's stronger bleed reduction takes priority
			amt *= BLOOD_RESISTANCE_EFFECTIVE_BLEEDRATE
		else if(HAS_TRAIT(src, TRAIT_CRITICAL_RESISTANCE))
